"""Integration tests for ToDoService.mark_to_do_as_done()."""
import datetime
import uuid
...
class TestMarkTodoDoneSuccessIntegration:
    """Integration tests for successful mark_to_do_as_done scenarios."""
    ...
    @pytest.mark.asyncio
    async def test_mark_as_done_preserves_title(self, todo_service, mock_repository):
        """Test mark_as_done preserves original title."""
        todo_id = uuid.uuid4()
        mock_entry = ToDoEntryData(
            id=todo_id,
    ...
    @pytest.mark.asyncio
    async def test_mark_as_done_preserves_description(self, todo_service, mock_repository):
        """Test mark_as_done preserves original description."""
        todo_id = uuid.uuid4()
        mock_entry = ToDoEntryData(
            id=todo_id,
            title="Title",
...
class TestMarkTodoDoneNotFoundIntegration:
    """Integration tests for mark_to_do_as_done not found scenarios."""

    @pytest.mark.asyncio
    async def test_mark_as_done_not_found(self, todo_service, mock_repository):
        """Test mark_as_done raises error when entry not found."""
        mock_repository.get_to_do_entry.return_value = None

        with pytest.raises(ToDoNotFoundError):
            await todo_service.mark_to_do_as_done(uuid.uuid4())
...
class TestMarkTodoDoneRepositoryIntegration:
    """Integration tests for mark_to_do_as_done repository interaction."""
    ...
    async def test_mark_as_done_updates_with_done_true(self, todo_service, mock_repository):
        """Test mark_as_done creates update payload with done=True."""
        todo_id = uuid.uuid4()
        mock_entry = ToDoEntryData(
            id=todo_id,
            title="Original",
    ...
    @pytest.mark.asyncio
    async def test_mark_as_done_passes_correct_todo_id(self, todo_service, mock_repository):
